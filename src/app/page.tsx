...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://5000-i87xo3h16skw08rkoxic2-b2fcf0e7.manus.computer"; // Use the exposed backend URL
...
export default function LoginPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.logged_in) {
            ...
            router.push("/leads"); // Redirect to leads page if already logged in
          }
        }
      } catch (err) {
    ...
    // Check for Google login error from query params
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Redirect to the dashboard or leads page after successful login
        router.push("/leads"); 
      } else {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRegisterEmail("");
        setRegisterPassword("");
        // Consider automatically logging in or prompting user to log in
      } else {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center">
          ...
          <CardDescription>Manage your leads and appointments efficiently</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="text-center text-xs text-muted-foreground">
          Protected and powered by PV Digital. This web-app does not store your data outside your credentials. It's safe, fast, robust & secure!
        </CardFooter>
      </Card>
    </div>
  );
